...
**Status: CLOSED / GO** — certified end-to-end in production by owner
human runtime checkpoint on September 16, 2026. See **DATA-1 — HUMAN
RUNTIME CERTIFICATION / FINAL CLOSEOUT** at the end of this document for
the closing evidence record; everything below this line is the phase-by-
phase build/activation history that led there.
...
## DATA-1 — GTM LIVE-DETECTION BUG (DIAGNOSIS + FIX)

Shortly after the Consent Activation deployment, the owner's own live
Tag Assistant / GTM Preview testing against `https://futuretutor.ca/en`
reported **"Non connecté" / "0 Balise Google détectée"** in two scenarios:
a fresh accept click, and — more decisively — a page loaded with consent
already `"granted"` from a prior visit.

### Diagnosis

A read-only diagnostic mission traced the full chain (`ConsentBanner` →
consent persistence → `hasAnalyticsConsent()` → the GTM loader → script
insertion → route/environment eligibility → `NEXT_PUBLIC_GTM_ID`
resolution) and found the exact root cause by both grepping the source
tree and downloading and grepping the actual compiled JS chunks served by
production:

- `NEXT_PUBLIC_GTM_ID` **was** correctly available at build time — the
  literal string `GTM-KCNP43TK` was found hardcoded (not a `process.env`
  reference) inside the compiled `loadGtmIfEligible` function in the live
  production bundle. This ruled out any build/runtime env-var propagation
  issue.
- No CSP exists (confirmed via live response headers) — nothing blocks
  `googletagmanager.com`.
- The www→apex redirect preserves the full query string, so GTM Preview's
  `?gtm_debug=...` parameter is never stripped.
- **The actual defect**: `loadGtmIfEligible()` was called from exactly one
  place in the entire application — the Accept button's `onClick` handler
  in `ConsentBanner.tsx`. There was no code path anywhere that checked
  `hasAnalyticsConsent()` and loaded GTM on a fresh page load/mount. A
  visitor whose consent was already `"granted"` from an earlier visit
  therefore never got GTM loaded at all — the banner correctly stayed
  hidden (as designed), but nothing else ever invoked the loader.

A secondary finding: `shouldLoadGtm()`/`loadGtmIfEligible()` check
hostname + GTM ID + consent, but **never** route eligibility — that
guarantee existed only incidentally, because the one call site was itself
unreachable on an ineligible route. Any new call site checking consent on
mount would need to independently re-enforce `isAnalyticsEligiblePath()`,
or it would risk loading GTM on a private route for a returning visitor.

### Fix

`src/lib/analytics/vendors.ts` gained one new exported function,
`loadGtmIfConsentAlreadyGranted(pathname)`, which explicitly re-checks
`isAnalyticsEligiblePath(pathname)` **and** `hasAnalyticsConsent()` before
calling the existing `loadGtmIfEligible()` — closing exactly the gap above
without weakening the private-route guarantee. `ConsentBanner.tsx` calls
it from a `useEffect(() => { loadGtmIfConsentAlreadyGranted(pathname); },
[pathname])` that runs on every mount and pathname change, so a returning
visitor with already-granted consent now gets GTM loaded on that route
without needing to re-click Accept. The pre-existing `handleAccept()` path
(immediate load on a fresh Accept click) is unchanged.

11 new focused tests in `vendors.test.ts` cover: undecided/denied/granted
on an eligible route, a pathname change across two eligible routes with no
duplicate script, exclusion on a private route, an auth-utility route, and
an admin route, EN and FR eligible routes, and a full revoke→unload→
no-reload round trip. Full non-DB-integration suite: 300/300 passing in
the analytics/marketing/legal scope, 2418 passing overall with zero new
regressions (the same 36 pre-existing DB-integration test files that need
a live Postgres connection this environment doesn't have were unaffected
and unchanged from before this fix).

`.env.example`'s GTM comment was corrected (documentation only, no
environment values changed) — it previously read as if no GTM container
had been created yet, which was stale since the Consent Activation phase.
...
**Human checkpoint — satisfied.** The owner confirmed, in a real browser
with a freshly-started GTM Preview session, that `gtm.js?id=GTM-KCNP4STK`
returns successfully, `window.google_tag_manager` is populated, GA4 fires,
and a certified event's parameters contain only allowlisted fields with no
PII. See **DATA-1 — HUMAN RUNTIME CERTIFICATION / FINAL CLOSEOUT** below
for the full evidence record.

---

## DATA-1 — HUMAN RUNTIME CERTIFICATION / FINAL CLOSEOUT

**Status: CLOSED / GO** (as of the owner's human runtime checkpoint,
September 16, 2026).

The owner performed a live production check in a real browser and
reported the following evidence:

- Analytics consent was **GRANTED**.
- Production Network panel: `gtm.js?id=GTM-KCNP4STK` → **HTTP 200**. The
  GTM bootstrap request also returned **HTTP 200**.
- Browser console: `window.google_tag_manager` returned a real,
  initialized object containing `GTM-KCNP4STK: {...}` — GTM runtime
  initialization confirmed.
- GA4 network requests were observed using Measurement ID
  `G-3BQQXRZHY2`; GA4 collect requests returned **HTTP 204**.
- A real certified FutureTutor event was inspected in the GA4 request
  payload: `en=find_tutor_cta_clicked`, `ep.cta_location=header` — an
  exact match to this mission's own certified event/property allowlist.
- The inspected production payload contained **no visible PII**: no
  email, person name, phone number, postal/street address, user
  identifier, or form/free-text content. The request also carried normal
  GA4/browser technical metadata (page URL, referrer, page title,
  client/session identifiers, browser/device information) — standard GA4
  infrastructure, not FutureTutor application PII.
- The previously diagnosed GTM container-ID incident (wrong:
  `GTM-KCNP43TK`, correct: `GTM-KCNP4STK`) is resolved — the correct
  container now loads and initializes successfully in production.

This is the first human confirmation that the full, real, consent-gated
analytics pipeline works end-to-end in production: consent → GTM load →
GA4 tag fire → a certified event → allowlisted, PII-free parameters.

### Non-blocking follow-ups (do not block this closure)

**P2 — `DATA-1-GTM-LOADER-HARDENING1`**: evaluate enforcing analytics
route eligibility inside the GTM loader itself (`shouldLoadGtm()` /
`loadGtmIfEligible()`), so a future call site cannot accidentally load GTM
on a private route by omitting its own `isAnalyticsEligiblePath()` check.
Today this guarantee holds because both existing call sites
(`ConsentBanner`'s click handler and its mount effect) independently
enforce it themselves — recorded during the GTM live-detection fix, not
yet implemented, not required for this closure.

**LEGAL**: the Cookie Policy's analytics-related text (§13/§14/§28/§45)
remains subject to the owner's/external Canadian legal review, per this
Policy's own standing "subject to final external Canadian legal review"
status. This does **not** block DATA-1's technical closure.
...
immediately after it, the GTM container ID correction that followed, and
the owner's human runtime certification that closes DATA-1. DATA-1
status: CLOSED / GO.*
