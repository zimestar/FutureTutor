...
- Diagnostic commit: `620abd8` (temporary, since fully reverted by content —
  no `seo_host_probe` code remains)
- Fix commit: `ddf40c0`
- Fix deployment: `82049354-c942-40f3-a034-085adfc7ba6d` (SUCCESS)

### Final live certification (2026-09-14)

Every tested `www` path — root, `/en`, `/fr`, `/en/find-tutors`,
`/fr/resources`, `/en/tutoring/edmonton`, `/en/subjects/math`,
`/en/resources/how-to-choose-a-tutor`, and a query-string case
(`?mode=online&utm_source=seo_test`) — returns a single-hop
`308 Permanent Redirect` to the exact matching apex URL, path and query
preserved. Zero duplicate 200 responses anywhere on `www`. Apex regression
confirmed healthy: `/en/find-tutors` 200, `/fr/resources` 200,
`/api/health` 200, `/api/health/ready` 200, canonical still
`https://futuretutor.ca/en`/`/fr`, both hreflang alternates present, zero
`www` URLs in `sitemap.xml`, `robots.txt` still points at
`https://futuretutor.ca/sitemap.xml`.
...
- DNS record: **created and live** — `www` CNAME → `q2vwb12u.up.railway.app`,
  confirmed via two independent public resolvers.
- Application redirect: **fixed, deployed, and live-certified** (see
  above). The first implementation (`ba55d53`) was live-certified broken;
  the `host`-header-based fix (`ddf40c0`) is confirmed correct against real
  production traffic.
- Apex canonical/hreflang/sitemap/robots: unaffected throughout, still
  `GO`.
- **Mission closed.** `www.futuretutor.ca` permanently redirects to the
  apex with no duplicate-content exposure.
