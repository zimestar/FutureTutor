...
// FINANCIAL-CONVERGENCE-ROUTE-SEPARATION1: markEligibleEarnings (and the
// DB-only convergence sweep it used to run alongside, sweepTutorEarningConvergence)
// now live in src/services/tutorEarningConvergence.ts — a module with zero
// Stripe imports anywhere — so the DB-only convergence/eligibility path is
// structurally, not just conventionally, incapable of reaching Stripe. This
// file keeps ONLY the Stripe-capable transfer boundary
// (createTransferForEarning) and the Payment/Refund reconciliation sweep
// (reconcileStuckPayments), both still reachable exclusively through the
// still-unscheduled /api/cron/payments-tick.
...
/**
 * The cron sweep's entry point for the TRANSFER half of reconciliation —
 * see reconcileStuckPayments below for the Payment/Refund half.
 *
 * FINANCIAL-CONVERGENCE-ROUTE-SEPARATION1: this function no longer runs the
 * DB-only financial convergence sweep or the eligibility-promotion sweep —
 * those moved to processFinancialConvergenceAndEligibility
 * (src/services/tutorEarningConvergence.ts), called from the new, separately-
 * schedulable /api/cron/session-financial-convergence-tick route. This
 * function now processes ONLY earnings that are ALREADY ELIGIBLE at the
 * moment it runs — it never converges or promotes anything itself. This is
 * intentional: it is what keeps this file (and the still-unscheduled
 * /api/cron/payments-tick that calls it) the ONLY reachable path to Stripe
 * transfer creation, while the DB-only convergence/eligibility work can run
 * on its own schedule with zero Stripe exposure.
 */
