    ...
    <section className="relative bg-[#f8f6f1]">
      <div className="relative xl:min-h-[690px]">
        <Container className="relative z-10 pb-8 pt-12 sm:pb-10 sm:pt-16 xl:pb-28 xl:pt-20">
          <div data-testid="hero-copy" className={isFrench ? "max-w-[590px]" : "max-w-[620px]"}>
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-blue md:text-sm">{t("eyebrow")}</p>
            <h1
              data-testid="hero-heading"
              className={`mt-5 text-balance font-sans font-extrabold tracking-[-0.05em] text-navy ${
                isFrench
                  ? "text-[clamp(2.55rem,7.5vw,4.35rem)] leading-[1.01] xl:text-[clamp(3.3rem,4.2vw,4.65rem)]"
                  : "text-[clamp(2.7rem,8vw,4.5rem)] leading-[0.98] xl:text-[clamp(4rem,5vw,4.75rem)]"
              }`}
            >
        ...
        <div
          ...
          className={`relative aspect-[16/10] w-full overflow-hidden sm:aspect-[16/8] xl:absolute xl:inset-y-0 xl:right-0 xl:aspect-auto ${isFrench ? "xl:w-[69%]" : "xl:w-[72%]"}`}
        >
          <Image
      ...
      <Container className="relative z-20 pb-10 xl:-mt-28">
        <div className="mx-auto max-w-6xl">
          ...
          <ul data-testid="hero-trust-row" className="mt-5 grid grid-cols-1 gap-x-6 gap-y-3 px-1 text-sm font-semibold text-navy/72 sm:grid-cols-2 min-[1280px]:grid-cols-4">
