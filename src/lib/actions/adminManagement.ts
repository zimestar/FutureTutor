...
import { ADMIN_PERMISSIONS, replaceAdminPermissions, requireSuperAdmin } from "@/services/adminPermissions";
...
export async function inviteAdminAction(_state: AdminManagementState, form: FormData): Promise<AdminManagementState> {
  ...
  let actor; try { actor = await requireSuperAdmin(session); } catch { return { error: "forbidden" }; }
...
export async function updateAdminPermissionsAction(adminId:string,_state:AdminManagementState,form:FormData){
  const session=await auth();
  let actor; try { actor = await requireSuperAdmin(session); } catch { return{error:"forbidden"}; }
  await replaceAdminPermissions(adminId,selectedPermissions(form),actor.id);revalidatePath(`/admin/admins/${adminId}`);return{success:true};
}
export async function suspendAdminAction(adminId:string){
  const session=await auth();
  const actor = await requireSuperAdmin(session); // throws FORBIDDEN on stale/deactivated/demoted SUPER_ADMIN — fresh every call
  if(adminId===actor.id)throw new Error("FORBIDDEN");
  await db.$transaction(async tx=>{const target=await tx.user.findUnique({where:{id:adminId},select:{role:true}});if(!target||target.role!=="ADMIN")throw new Error("FORBIDDEN");await tx.user.update({where:{id:adminId},data:{deactivatedAt:new Date()}});await tx.auditLog.create({data:{actorUserId:actor.id,action:"admin.suspended",entityType:"User",entityId:adminId}})});revalidatePath("/admin/admins");
}
export async function reactivateAdminAction(adminId:string){
  const session=await auth();
  const actor = await requireSuperAdmin(session);
  await db.$transaction(async tx=>{await tx.user.update({where:{id:adminId,role:"ADMIN"},data:{deactivatedAt:null}});await tx.auditLog.create({data:{actorUserId:actor.id,action:"admin.reactivated",entityType:"User",entityId:adminId}})});revalidatePath("/admin/admins");
}
export async function revokeInvitationAction(invitationId:string){
  const session=await auth();
  const actor = await requireSuperAdmin(session);
  await db.$transaction(async tx=>{await tx.adminInvitation.update({where:{id:invitationId,acceptedAt:null},data:{revokedAt:new Date()}});await tx.auditLog.create({data:{actorUserId:actor.id,action:"admin.invitation_revoked",entityType:"AdminInvitation",entityId:invitationId}})});revalidatePath("/admin/admins");
}

export async function resendInvitationAction(invitationId: string, locale: "en" | "fr") {
  const session = await auth();
  const actor = await requireSuperAdmin(session);
  const { invitation, rawToken } = await resendAdminInvitation(db, invitationId, actor.id);
...
export async function promoteAdminToSuperAdminAction(adminId: string) {
  const session = await auth();
  const actor = await requireSuperAdmin(session); // fresh: a demoted/deactivated SUPER_ADMIN can never mint a new one
  if (adminId === actor.id) throw new Error("FORBIDDEN");
  await db.$transaction(async (tx) => {
    ...
    await tx.auditLog.create({ data: { actorUserId: actor.id, action: "admin.promoted_to_super_admin", entityType: "User", entityId: adminId } });
  });
